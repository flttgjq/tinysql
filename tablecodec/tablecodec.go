...
// record code:t_tableId_r_rowId
// index code:t_tableId_i_indexId_ColumnsValue_rowId
var (
	tablePrefix     = []byte{'t'}
...
func DecodeRecordKey(key kv.Key) (tableID int64, handle int64, err error) {
	...
	if !key.HasPrefix(tablePrefix) {
		return 0, 0, errInvalidRecordKey.GenWithStack("invalid record key:%q tablePrefix mismatch", key)
	}
	...
	if !key.HasPrefix(recordPrefixSep) {
		return 0, 0, errInvalidRecordKey.GenWithStack("invalid record key:%q recordPrefixSep mismatch", key)
	}
...
func DecodeIndexKeyPrefix(key kv.Key) (tableID int64, indexID int64, indexValues []byte, err error) {
	...
	if !key.HasPrefix(tablePrefix) {
		return 0, 0, nil, errInvalidIndexKey.GenWithStack("invalid index key:%q tablePrefix mismatch", key)
	}
	key = key[len(tablePrefix):]
	key, tableID, err = codec.DecodeInt(key)
	if err != nil {
	...
	if !key.HasPrefix(indexPrefixSep) {
		return 0, 0, nil, errInvalidIndexKey.GenWithStack("invalid index prefix:%q", key)
	}
